...
*/
		...
		try {
			...
			if (!result.next()) {
				...
				if (item.getUser().getLocation() != null) {
					location = item.getUser().getLocation(); // location of the
																// user
				}
	...
	private void visualQuery(Status item) {
		try {
			if (item.getMediaEntities().length != 0) {// check if there is
														// visual url
	...
	/*
	  private void republishQuery(Status item) { try { IDs id; id =
	  twitter.getRetweeterIds(item.getId(), -1); long[] ids = id.getIDs();
	  PreparedStatement republishQuery; republishQuery = connection.
	  prepareStatement("INSERT INTO `republish`(`item_id`,`user_id`) VALUES (?,?)"
	  ); for (int i = 0; i < ids.length; i++) {
	  
	  republishQuery.setString(1, item.getId() + "");
	  republishQuery.setString(2, ids[i] + ""); republishQuery.executeUpdate();
	 System.out.println("republishQuery"); }
	  ...
	  private void likeQuery(String item_id, String user_id){ try{
	  
	  PreparedStatement likeQuery; likeQuery = connection.
	  prepareStatement("INSERT INTO `like`(`item_id`,`user_id`) VALUES (?,?)");
	  likeQuery.setString(1, item_id); likeQuery.setString(2, user_id);
	  likeQuery.executeUpdate(); System.out.println("likeQuery");
	...
	private void tagsQuery(Status item) {
		try {
			...
			if (item.getHashtagEntities().length != 0) {// check if there is
														// hashtags
	...
	public void storStatus1(Status status) {
		...
		// String tweetQuery ="INSERT INTO `item`(`item_id`, `textual_content`,
		// `visual_content`, `geotag`, `timestamp`) "
		// + "VALUES
		// ("+item.getItemId()+",'"+item.getTextualContent()+"','"+item.getVisualContent()+"','"+item.getGeoTags()+"','"+item.getTimestamp()+"')";
